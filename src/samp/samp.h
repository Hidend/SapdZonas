...
namespace offsets {
	...
	namespace game {
		...
		constexpr std::uintptr_t create_object[] = { 0x9B890, 0x9BA10, 0x9FC20, 0xA0360, 0x9E500, 0xA0170 };
	}
}
...
public:
	...
	void create_object(int modelId, float x, float y, float z, int rotationX, int rotationY, int rotationZ, float drawDistance = 0.0f)
	{
		((int(__thiscall*)(c_game*, int, float, float, float, int, int, int, float))
			rakhook::samp_addr(offsets::game::create_object[VERSION]))
			(this, modelId, x, y, z, rotationX, rotationY, rotationZ, drawDistance);
	}
};
